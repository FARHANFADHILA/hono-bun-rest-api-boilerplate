...
export const createUniversitasSchema = z.object({
  nama: z
    .string({
      message:
        "Nama universitas harus berupa text, bukan angka atau tipe data lain",
    })
    .trim()
  ...
  created_by: z
    .string()
    .uuid({ message: "Format UUID tidak valid untuk created_by" })
    .optional(),

  updated_by: z
    .string()
    .uuid({ message: "Format UUID tidak valid untuk updated_by" })
    .optional(),
});
